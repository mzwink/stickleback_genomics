...
def remove_fq_seq(random_seq_identifier, num_reads_remove):
    for i in range(num_reads_remove):
        random_seq_identifier.remove(random_seq_identifier[1])
        print(len(random_seq_identifier))
...
def parse_read_counts(read_info_file, indiv_pop_num):
    print("parsing read counts")
    num_reads = open(read_info_file).readlines()
    indiv_pop = int(indiv_pop_num)
...
#### Determines number reads to remove, outputs new ##
...
def downsize_LW_pop():
    ...
    fq_directories = glob.glob(directory + "LW_*/")

    for lw_dir in fq_directories:

        # All lanes were combined to one fastq for each indiv
        fq_files = glob.glob(lw_dir + "LW*R1_001.fastq")

        for fq in fq_files:
            print(fq)

            # Find the individual number
            indiv = fq.replace(lw_dir, "")
            indiv = indiv.split("_")[1]
            reads_to_remove = parse_read_counts(remove_lines, indiv)
            ...
            print("Removing " + str(reads_to_remove) + " reads from " + str(fq))

            seq_identifier = []

            # If more PS reads than LW, don't downsize fq file
            # Append sequence identifiers to array to randomly choose reads for output
            ...
            #shuffle array (randomize) and remove reads
                print("All sequence identifiers loaded into array.")
                #random_seq_identifier = random.shuffle(seq_identifier)
                random.shuffle(seq_identifier)

                print("Sequence identifier array shuffled. Removing...")
                output_reads = remove_fq_seq(seq_identifier, reads_to_remove)
                print(str(reads_to_remove) + " were removed from array.")
                R1.close()
                R2.close()
